...
{
  countDropped += p->GetSize();
  theTime = Simulator::Now().GetSeconds();
 // NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}
static void
RxEnd (Ptr<const Packet> p)
{
  countReceived += p->GetSize();
  theTime = Simulator::Now().GetSeconds();
...
//-----------------------------------------------------------------
  ...
  double throughput = (countReceived+countDropped) / theTime;
  std::cout <<currentNoise << " " << throughput << std::endl; 
  ...
  GnuplotCollection gnuplots ("OOKErrorModel-Graph.pdf");
 {
 // plot.setTitle("ns3::OOKErrorModel");
