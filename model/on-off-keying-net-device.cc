...
#ifndef ON_OFF_KEYING_NET_DEVICE_H
#define ON_OFF_KEYING_NET_DEVICE_H
...
/**
 * \ingroup on-off-keying
 * \class OnOffKeyingNetDevice
 * \brief A Device for a OOK Network Link.
 *
 * This OnOffKeyingNetDevice class specializes the NetDevice abstract
 * base class.  Together with a OOKChannel (and a peer 
 * OnOffKeyingNetDevice), the class models, with some level of 
 * abstraction, a generic on-off-keying or serial link.
 * Key parameters or objects that can be specified for this device 
 * include a queue, data rate, and interframe transmission gap (the 
 * propagation delay is set in the OOKChannel).
 */
class OnOffKeyingNetDevice : public NetDevice
{
public:
  /**
  ...
  /**
   * Construct a OnOffKeyingNetDevice
   *
   * This is the constructor for the OnOffKeyingNetDevice.  It takes as a
   * parameter a pointer to the Node to which this device is connected, 
   * as well as an optional DataRate object.
   */
  OnOffKeyingNetDevice ();

  /**
   * Destroy a OnOffKeyingNetDevice
   *
   * This is the destructor for the OnOffKeyingNetDevice.
   */
  virtual ~OnOffKeyingNetDevice ();
   ...
   *
   * The OnOffKeyingNetDevice "owns" a queue that implements a queueing 
  ...
  /**
   * Attach a receive ErrorModel to the OnOffKeyingNetDevice.
   *
   * The OnOffKeyingNetDevice may optionally include an ErrorModel in
   ...
   *
   * The OnOffKeyingNetDevice receives packets from its connected channel
...
private:
   ...
   */
  OnOffKeyingNetDevice& operator = (const OnOffKeyingNetDevice &o);
   ...
   * \param o Other NetDevice
   */
  OnOffKeyingNetDevice (const OnOffKeyingNetDevice &o);
...
private:
   ...
   * through the on-off-keying channel.
   */
  ...
  /**
   * The OOKChannel to which this OnOffKeyingNetDevice has been
   * attached.
   */
  Ptr<OOKChannel> m_channel;

  /**
   * The Queue which this OnOffKeyingNetDevice uses as a packet source.
   * Management of this Queue has been delegated to the OnOffKeyingNetDevice
   ...
   * here in the on-off-keying device).
   */
  ...
  /**
   * \brief OOK to Ethernet protocol number mapping
   * \param protocol A OOK protocol number
   * \return The corresponding Ethernet protocol number
   */
  static uint16_t OokToEther (uint16_t protocol);

  /**
   * \brief Ethernet to OOK protocol number mapping
...
#endif /* ON_OFF_KEYING_NET_DEVICE_H */
